...
async function fetchMediaForDefault(url, overrideBase = null, timeout = 7000) {
  let backendBase;
  if (overrideBase) {
    backendBase = overrideBase;
  } else {
  ...
  setUrlStatus("ok", `Sukses memuat lagu default: ${data.title}`);
  
  // Set source but don't force playback to prevent autoplay block
  initAudio();
  if (audioCtx && audioCtx.state === "suspended") {
    // Keep it suspended or resume, but don't play immediately
  }
  audioEl.src = data.streamUrl;
  audioEl.load();
  updateMeta(data.title, data.artist);
  
  // Try to play (will catch the error if browser blocks it)
  audioEl.play().then(() => {
    isPlaying = true;
    document.getElementById("playBtn").textContent = "❚❚";
    if (!rafId) drawLoop();
  }).catch(playErr => {
    console.log("Autoplay prevented as expected, waiting for user interaction:", playErr);
    isPlaying = false;
    document.getElementById("playBtn").textContent = "▶";
    setUrlStatus("ok", `Lagu default siap diputar: ${data.title}. Klik tombol Play!`);
  });
}
